...
public:
  ...
  void operator=(std::vector<T> &V) { vec = V; }
  void operator=(T val) { std::fill(vec.begin(), vec.end(), val); }
  ...
  std::vector<T> get_vector() const { return vec; }
  std::pair<std::size_t, std::size_t> &dim() { return d; }
  std::pair<std::size_t, std::size_t> dim() const { return d; }
...
template <typename T> Matrix<T> operator+(const T val, const Matrix<T> &rhs) {
...
template <typename T> Matrix<T> operator+(const T val, Matrix<T> &&rhs) {
  std::pair<std::size_t, std::size_t> temp_d = rhs.dim();
  for (int i = 0; i < temp_d.first; i++) {
    for (int j = 0; j < temp_d.second; j++) {
      rhs(i, j) = val + rhs(i, j);
...
template <typename T> Matrix<T> operator+(const Matrix<T> &rhs, const T val) {
  return val + rhs;
}
template <typename T> Matrix<T> operator+(Matrix<T> &&rhs, const T val) {
...
template <typename T> std::vector<T> get_vec(std::string S, char sep) {
  std::vector<T> vec;
  T val;
  std::string word;
...
template <typename T> Matrix<T> read_from_file(std::string filename) {
  std::ifstream fin;
  std::string line;
  std::vector<T> vec;
...
template <typename T> void init_rand(Matrix<T> &M, std::size_t seed) {
...
template <typename T> void init_eye(Matrix<T> &M, std::size_t seed) {
  std::size_t row = M.dim().first;
  std::size_t col = M.dim().second;
  // srand(time(NULL));
  for (std::size_t i = 0; i < row; i++) {
...
template <typename T>
T Filter(Matrix<T> &image, Matrix<T> &filter, std::size_t row,
         std::size_t col) {
  T res = T{};
...
template <typename T> Matrix<T> mirror_image(Matrix<T> &m) {
...
template <typename T> void rearrange(Matrix<T> &m) {
  std::size_t row = m.dim().first;
  std::size_t col = m.dim().second;
  for (int i = 0; i < row; i++) {
    if (m(i, i) == T{}) {
...
template <typename T> void row_mul(Matrix<T> &m, std::size_t row, T val) {
...
template <typename T> Matrix<T> inverse(Matrix<T> &m) {
...
template <typename T> void Upper_diogonlised(Matrix<T> &m) {
  std::size_t row = m.dim().first;
  std::size_t col = m.dim().second;
  rearrange(m);
  for (int i = 0; i < row; i++) {
...
template <typename T> Matrix<T> liner_equ_solver(Matrix<T> &m) {
  std::size_t row = m.dim().first;
  std::size_t col = m.dim().second;
  Matrix<T> X(row, 1);
  T temp;
...
template <typename T, typename Fun> void Transform(Matrix<T> &M, Fun f) {
...
void Transform(Matrix<T> &lhs, Matrix<T> &rhs, Matrix<T> &res, Fun f) {
  ...
  std::transform(vec_lhs.begin(), vec_lhs.end(), vec_rhs.begin(),
                 vec_res.begin(), f);
}
template <typename T> Matrix<T> Cholesky_decomposition(Matrix<T> &m) {
  std::size_t row = m.dim().first;
  std::size_t col = m.dim().second;
...
template <typename T> Matrix<T> mean(Matrix<T> &M, bool flag = false) {
  Matrix<T> res;
  if (!flag) {
    res.set_dim(1, 1);
    res(0, 0) = std::accumulate(M.get_vector().begin, M.get_vector().end) /
                (M.dim().first * M.dim().second);
  } else {
...
template <typename T> Matrix<T> varience(Matrix<T> &M, bool flag = false) {
  Matrix<T> mu = mean(M, flag);
  Matrix<T> res;
  if (!flag) {
    res(0, 0) = std::accumulate(
        M.get_vector().begin, M.get_vector().end,
        [&](T x, T y) { return x + std::pow(y - mu(0, 0), 2); });
  } else {
    res.set_dim(1, M.dim().first);
    for (int i = 0; i < M.dim().second; i++) {
      res(0, i) =
          row_op(M, [&](T x, T y) { return x + std::pow(y - mu(0, i), 2); });
      res(0, i) = res(0, i) / M.dim().second;
    }
  }
  return res;
...
template <typename T>
std::pair<Matrix<T>, Matrix<T>> normalization(Matrix<T> &M, T gemma = 1,
                                              T beta = 0, T epsolon = 0,
                                              bool flag = false) {
  Matrix<T> m, v;
  ...
  if (!flag) {
    ...
    Transform(M, out, [&](T x) {
      return (x - m(0, 0)) / std::sqrt(v(0, 0) + epsolon);
    });
    res(0, 0) = m(0, 0);
    res(1, 0) = v(0, 0);
  } else {
    ...
    for (int i = 0; i < M.dim().second; i++) {
      row_transform(M, i, [&](T x) {
        return (x - m(0, i)) / std::sqrt(v(0, i) + epsolon);
      });
    }
  }
  for (int i = 0; i < M.dim().second; i++) {
  ...
  res.second(2, 0) = gemma;
  res.second(2, 1) = beta;
  res.second(2, 2) = epsolon;
  res.first = gemma * out + beta;
  return res;
}
template <typename T, typename Fun> Matrix<T> math_fun(Matrix<T> &M, Fun f) {
  Matrix<T> res(M.dim().first, M.dim().second);
  Transform(M, res, f);
  return res;
}
template <typename T> Matrix<T> tanh(Matrix<T> &M) {
  return math_fun(M, [](T x) { return std::tanh(x); });
}
template <typename T> Matrix<T> pow(Matrix<T> &M, T n) {
  return math_fun(
      M, [&](T x) { return n > 0 ? std::pow(x, n) : (1.0 / std::pow(x, n)); });
}
template <typename T> Matrix<T> sigmoid(Matrix<T> &M) {
